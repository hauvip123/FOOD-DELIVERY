...
const variantClasses = {
  ...
  light:
    "bg-white/95 text-[#23140c] ring-black/5 hover:bg-red-50 hover:text-red-500",
...
export function FavoriteRestaurantButton({
  ...
  return (
    <button
      ...
      aria-label={
        isFavorite ? "Bỏ yêu thích nhà hàng" : "Thêm nhà hàng yêu thích"
      }
      title={isFavorite ? "Bỏ yêu thích" : "Yêu thích"}
      className={`grid place-items-center rounded-2xl ring-1 transition-all disabled:pointer-events-none disabled:opacity-60 active:scale-90 ${size === "lg" ? "size-14 rounded-2xl" : "size-12"} ${isFavorite ? "bg-red-500 text-white ring-red-500 hover:bg-red-600" : variantClasses[variant]}`}
    >
      <Heart
        size={size === "lg" ? 28 : 24}
