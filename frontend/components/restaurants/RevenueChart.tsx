...
function RevenueChart({
  ...
  data: ManagedOverviewResponse["revenueSeries"];
}) {
  ...
  return (
    <div className="rounded-4xl bg-white p-6 shadow-[0_22px_55px_-34px_rgba(35,20,12,0.38)] ring-1 ring-black/5">
      ...
      <div className="overflow-hidden rounded-3xl bg-[#fffcf8] p-3 ring-1 ring-[#23140c]/5">
        <svg
          viewBox="0 0 640 240"
          ...
          {points.map((point) => (
            <g key={point.date}>
              ...
              <text
                ...
                y="226"
                textAnchor="middle"
                className="fill-[#704322] text-[12px] font-bold"
              >
      ...
      <div className="mt-4 grid gap-2 sm:grid-cols-7">
        {data.map((item) => (
          <div
            key={item.date}
            className="rounded-2xl bg-[#fffcf8] px-3 py-2 ring-1 ring-[#23140c]/5"
          >
            <p className="text-[10px] font-black uppercase tracking-widest text-[#704322]/35">
              {shortDate(item.date)}
            </p>
            <p className="mt-1 truncate text-xs font-black text-[#23140c]">
