...
function StatusChart({
  ...
  data: ManagedOverviewResponse["statusBreakdown"];
  total: number;
}) {
  const maxCount = Math.max(...data.map((item) => item.count), 1);

  return (
    <div className="rounded-4xl bg-white p-6 shadow-[0_22px_55px_-34px_rgba(35,20,12,0.38)] ring-1 ring-black/5">
      ...
      <div className="space-y-4">
        {data.length === 0 ? (
          <div className="rounded-3xl border border-dashed border-[#23140c]/10 px-4 py-14 text-center">
            <p className="text-sm font-bold text-[#704322]/45">
              Chưa có đơn hàng để vẽ biểu đồ.
            </p>
          </div>
        ) : (
          data.map((item) => {
            ...
            return (
              <div key={item.status}>
                <div className="mb-2 flex items-center justify-between gap-3">
                  <span
                    className={`rounded-full px-3 py-1 text-xs font-black ring-1 ${statusTone[item.status] ?? "bg-stone-50 text-stone-700 ring-stone-100"}`}
                  >
                    {item.label}
                  </span>
                ...
                <p className="mt-1 text-[11px] font-bold text-[#704322]/40">
                  {percent}% tổng đơn
                </p>
              </div>
            );
