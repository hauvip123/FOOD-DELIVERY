...
const tagColors = ["bg-orange-500", "bg-red-500", "bg-emerald-500", "bg-pink-500", "bg-amber-600", "bg-sky-600"];
...
export function FeaturedRestaurants({ restaurants = [], isLoading = false }: FeaturedRestaurantsProps) {
  const displayRestaurants = restaurants.length > 0 ? buildRestaurants(restaurants) : fallbackRestaurants;
        ...
        {isLoading && restaurants.length === 0 ? (
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {Array.from({ length: 3 }).map((_, index) => (
              <div key={index} className="h-96 animate-pulse rounded-[2.5rem] bg-orange-50" />
            ))}
          </div>
        ) : (
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {displayRestaurants.map((res, idx) => (
              <motion.article
                ...
                className={"group relative flex flex-col overflow-hidden rounded-[2.5rem] bg-white p-4 shadow-[0_20px_50px_-20px_rgba(0,0,0,0.1)] ring-1 ring-black/5 transition-all hover:shadow-[0_30px_60px_-15px_rgba(255,107,0,0.2)] " + (idx === 0 ? "lg:col-span-2 lg:flex-row lg:gap-8 " : "") + (!res.isOpen ? "grayscale-[0.4] opacity-80" : "")}
              >
                <div className={"relative overflow-hidden rounded-[2rem] " + (idx === 0 ? "h-64 lg:h-auto lg:w-[45%]" : "h-64 w-full")}>
                  <Image
                    src={res.image}
                    alt={res.name}
                    fill
                ...
                <div className={"flex flex-1 flex-col justify-between py-4 " + (idx === 0 ? "lg:py-6" : "")}>
                    ...
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex min-w-0 flex-col gap-1">
                        <div className="flex items-center gap-2">
                          ...
                          <h3 className={"font-black tracking-tight text-[#23140c] " + (idx === 0 ? "text-3xl" : "text-2xl")}>
                            {res.name}
                          </h3>
                        </div>
                  ...
                  <Link
                    ...
                    className={"mt-8 inline-flex h-14 w-full items-center justify-center rounded-2xl text-sm font-black transition-all active:scale-98 lg:w-fit lg:px-10 " + (res.isOpen ? "bg-[#23140c] text-white hover:bg-[#ff6b00]" : "pointer-events-none bg-[#704322]/10 text-[#704322]/40")}
                  >
