...
export default function AdminOverviewPage() {
  ...
  const maxStatusCount = useMemo(
    () =>
      Math.max(
        ...(overview?.restaurantsByStatus.map((item) => item.count) ?? [1]),
        1,
      ),
    [overview],
  );
    ...
    >
      ...
      <header className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
        <div>
          ...
          <h1 className="mt-3 text-5xl font-black tracking-tight text-stone-900 md:text-6xl">
            Thống kê hệ thống
          </h1>
          <p className="mt-4 max-w-2xl text-base font-bold leading-relaxed text-slate-400">
            Quản trị viên có thể theo dõi sự phát triển của người dùng và hiệu
            quả kinh doanh của các nhà hàng trong thời gian thực.
          </p>
        </div>
      ...
      {isLoading ? (
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2 xl:grid-cols-4">
          {Array.from({ length: 4 }).map((_, i) => (
            <div
              key={i}
              className="h-64 animate-pulse rounded-[2.5rem] bg-white ring-1 ring-slate-100"
            />
          ))}
        ...
        <BentoCard className="flex h-96 flex-col items-center justify-center text-center">
          ...
          <p className="mt-2 text-slate-400 font-bold">
            Hãy quay lại sau khi hệ thống có hoạt động mới.
          </p>
        </BentoCard>
      ) : (
        <>
                ...
                <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    {overview.usersByStatus.map((item) => (
                      <div
                        key={item.status}
                        className="flex flex-col justify-center rounded-4xl bg-slate-50 p-6 transition-colors hover:bg-orange-50/50"
                      >
                        <p className="font-mono text-3xl font-black text-stone-900">
                          {item.count}
                        </p>
              ...
              <div className="space-y-8">
                {overview.restaurantsByStatus.map((item) => (
                  <StatBar
                    ...
                    max={maxStatusCount}
                    color={
                      item.status === "open" ? "bg-orange-500" : "bg-slate-300"
                    }
                  />
                ))}
                  ...
                  >
                    <div className="flex items-center gap-4">
                      ...
                      <div className="min-w-0">
                        <p className="truncate text-sm font-black text-stone-900 group-hover/item:text-orange-600 transition-colors">
                          {user.username}
                        </p>
              ...
              <div className="divide-y divide-slate-100">
                {overview.recentRestaurants.map((res) => (
                  ...
                  >
                    ...
                    <div className="flex flex-col items-end gap-2">
                      <div
                        className={`flex items-center gap-1.5 rounded-full px-3 py-1 text-[9px] font-black uppercase tracking-wider ring-1 ${res.isOpen ? "bg-orange-50 text-orange-700 ring-orange-100" : "bg-slate-50 text-slate-500 ring-slate-100"}`}
                      >
