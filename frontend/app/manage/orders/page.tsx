...
const statusConfig: Record<string, { label: string; tone: string; action?: { label: string; next: string } }> = {
  ...
  confirmed: { label: "Đã xác nhận", tone: "bg-blue-50 text-blue-700 ring-blue-100", action: { label: "Bắt đầu chuẩn bị", next: "preparing" } },
  preparing: { label: "Đang chuẩn bị", tone: "bg-orange-50 text-orange-700 ring-orange-100", action: { label: "Chuyển giao hàng", next: "delivering" } },
  delivering: { label: "Đang giao", tone: "bg-sky-50 text-sky-700 ring-sky-100" },
  completed: { label: "Đã nhận hàng", tone: "bg-emerald-50 text-emerald-700 ring-emerald-100" },
...
export default function ManageOrdersPage() {
  ...
  const pendingCount = useMemo(() => orders.filter((order) => order.orderStatus === "pending").length, [orders]);
  const activeCount = useMemo(() => orders.filter((order) => !["completed", "cancelled"].includes(order.orderStatus)).length, [orders]);
  const todayRevenue = useMemo(() => {
    const today = new Date().toDateString();
    return orders
      .filter((order) => order.orderStatus === "completed" && new Date(order.createdAt).toDateString() === today)
  ...
  return (
    <div className="space-y-8">
      <header className="flex flex-col gap-5 md:flex-row md:items-end md:justify-between">
        <div>
          <p className="text-xs font-black uppercase tracking-widest text-[#ff6b00]">Quản lý đơn</p>
          <h1 className="mt-2 text-4xl font-black tracking-tight text-[#23140c]">Xác nhận đơn hàng</h1>
          <p className="mt-3 max-w-2xl text-sm font-bold leading-relaxed text-[#704322]/60">Xem đơn mới từ khách, xác nhận và chuyển đơn sang giao hàng. Hoàn thành sẽ do khách xác nhận.</p>
        </div>
      </header>
        ...
        <div className="rounded-[2rem] bg-white p-6 ring-1 ring-black/5">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="mt-1 text-xs font-black uppercase tracking-widest text-[#704322]/45">Chờ xác nhận</p>
            </div>
        ...
        <div className="rounded-[2rem] bg-white p-6 ring-1 ring-black/5">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="mt-1 text-xs font-black uppercase tracking-widest text-[#704322]/45">Đang xử lý</p>
            </div>
        ...
        <div className="rounded-[2rem] bg-[#23140c] p-6 text-white ring-1 ring-black/5">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="mt-1 text-xs font-black uppercase tracking-widest text-white/35">Đã nhận hôm nay</p>
            </div>
      ...
      {errorMessage && (
        <div className="rounded-[1.5rem] bg-red-50 px-5 py-4 text-sm font-bold text-red-700 ring-1 ring-red-100">
          {errorMessage}
        </div>
      )}
              ...
              <article key={order.id} className="rounded-[2rem] bg-white p-5 shadow-[0_18px_45px_-28px_rgba(35,20,12,0.35)] ring-1 ring-black/5 sm:p-7">
                <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
                  <div>
                    ...
                    <div className="mt-3 flex flex-wrap items-center gap-3 text-sm font-bold text-[#704322]/60">
                      ...
                      <span>•</span>
                      <Link href={`/manage/restaurants/${order.restaurantId}`} className="inline-flex items-center gap-1 text-[#ff6b00] hover:text-[#e45f00]">
                ...
                <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                  <div className="text-sm font-bold text-[#704322]/55">
                    Tạm tính {formatMoney(order.subtotal)} • Phí giao {formatMoney(order.deliveryFee)}
                  </div>
