      ...
      <header className="flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between">
        <div>
          ...
          <p className="mt-3 max-w-2xl text-sm font-bold leading-relaxed text-[#704322]/60">
            Theo dõi doanh thu đã được khách xác nhận, đơn đang xử lý và hiệu
            suất từng nhà hàng.
          </p>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="rounded-4xl border border-dashed border-[#23140c]/10 bg-white px-6 py-20 text-center">
          ...
          <p className="mt-3 text-sm font-bold text-[#704322]/55">
            Khi có nhà hàng và đơn hàng, số liệu sẽ xuất hiện tại đây.
          </p>
        </div>
      ) : (
        <>
              ...
              return (
                ...
                >
                  <div className="flex items-start justify-between gap-4">
                    ...
                    <span className="rounded-full bg-[#fffcf8] px-3 py-1 text-[10px] font-black uppercase tracking-widest text-[#704322]/35 ring-1 ring-[#23140c]/5">
                      Live
                    </span>
                  </div>
                  <p className="mt-6 text-xs font-black uppercase tracking-widest text-[#704322]/40">
                    {stat.label}
                  </p>
                  <p className="mt-2 truncate text-2xl font-black tracking-tight text-[#23140c]">
                    {stat.value}
                  </p>
                  <p className="mt-2 text-xs font-bold text-[#704322]/45">
                    {stat.note}
                  </p>
                </motion.div>
              );
          ...
          <div className="grid gap-6 xl:grid-cols-[1.45fr_0.85fr]">
            ...
            <StatusChart
              ...
              total={overview.summary.totalOrders}
            />
          </div>
          ...
          <div className="grid gap-6 xl:grid-cols-[0.9fr_1.1fr]">
                ...
                {overview.topRestaurants.length === 0 ? (
                  <div className="rounded-3xl border border-dashed border-[#23140c]/10 px-4 py-14 text-center">
                    <Storefront
                      ...
                      className="mx-auto text-orange-200"
                    />
                    <p className="mt-3 text-sm font-bold text-[#704322]/45">
                    ...
                    >
                      ...
                      <div className="min-w-0">
                        <div className="flex items-center gap-2">
                          ...
                          <span
                            className={`size-2 rounded-full ${restaurant.isOpen ? "bg-emerald-500" : "bg-stone-300"}`}
                          />
                        </div>
                        <p className="mt-1 text-xs font-bold text-[#704322]/45">
                          {restaurant.orderCount} đơn •{" "}
                          {restaurant.ratingAverage.toFixed(1)} sao
                        </p>
                      </div>
                      <div className="text-right">
                        <p className="text-sm font-black text-[#23140c]">
                          {formatCompactMoney(restaurant.revenue)}
                        </p>
                        <ArrowRight
                          size={16}
                          weight="bold"
                          className="ml-auto mt-1 text-[#704322]/25 transition-transform group-hover:translate-x-1 group-hover:text-[#ff6b00]"
                        />
                      </div>
                    </Link>
                  ))
            ...
            <section className="rounded-4xl bg-white p-6 shadow-[0_22px_55px_-34px_rgba(35,20,12,0.38)] ring-1 ring-black/5">
              <div className="mb-5 flex items-start justify-between gap-4">
                ...
                <Link
                  href="/manage/orders"
                  className="inline-flex items-center gap-1 text-xs font-black uppercase tracking-widest text-[#ff6b00] hover:text-[#e45f00]"
                >
                  Xem tất cả
                  ...
                  overview.recentOrders.map((order) => (
                    ...
                    >
                      <div className="min-w-0">
                        <div className="flex flex-wrap items-center gap-2">
                          <Link
                            ...
                            className="text-sm font-black text-[#23140c] hover:text-[#ff6b00]"
                          >
                            Đơn #{order.id}
                          </Link>
                          <span
                            className={`rounded-full px-2.5 py-1 text-[10px] font-black ring-1 ${statusTone[order.orderStatus] ?? "bg-stone-50 text-stone-700 ring-stone-100"}`}
                          >
                            {overview.statusBreakdown.find(
                              (item) => item.status === order.orderStatus,
                            )?.label ?? order.orderStatus}
                          </span>
                        </div>
                        <p className="mt-1 truncate text-xs font-bold text-[#704322]/45">
                          {order.restaurantName} • {order.phoneNumber} •{" "}
                      ...
                      <div className="text-left md:text-right">
                        ...
                        <p className="mt-1 text-[10px] font-black uppercase tracking-widest text-[#704322]/35">
