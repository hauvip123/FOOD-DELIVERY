...
import { ArrowRight, ChatCircleText, Clock, ForkKnife, PaperPlaneTilt, Storefront, UserCircle } from "@phosphor-icons/react";
...
function conversationTitle(conversation: ChatConversation, currentUserId?: number) {
...
function conversationSubtitle(conversation: ChatConversation, currentUserId?: number) {
  if (conversation.userId === currentUserId) {
    return conversation.restaurant?.address ?? conversation.restaurant?.city ?? "Nhà hàng";
  }
    ...
    async function loadConversations() {
      ...
      try {
        ...
        if (!Number.isNaN(restaurantIdFromQuery) && restaurantIdFromQuery > 0) {
          const conversation = await startRestaurantConversation(restaurantIdFromQuery);
          preferredConversationId = conversation.id;
          const exists = nextConversations.some((item) => item.id === conversation.id);
          nextConversations = exists
            ? nextConversations.map((item) => item.id === conversation.id ? conversation : item)
            : [conversation, ...nextConversations];
        }

        if (isCurrentRequest) {
          setConversations(nextConversations);
          setSelectedConversationId((currentId) => preferredConversationId ?? currentId ?? nextConversations[0]?.id ?? null);
        }
      } catch (error) {
        if (isCurrentRequest) {
          setConversations([]);
          setErrorMessage(error instanceof ApiError ? error.message : "Không thể tải tin nhắn.");
        }
      } finally {
        if (isCurrentRequest) {
  ...
  useEffect(() => {
    ...
    async function loadMessages(showLoading = false) {
      ...
      try {
        ...
        if (isCurrentRequest) {
          setErrorMessage(error instanceof ApiError ? error.message : "Không thể tải nội dung chat.");
        }
      } finally {
        if (isCurrentRequest) {
    ...
    return (
      <div className="flex min-h-[80dvh] items-center justify-center bg-[#fffcf8] px-4 text-center">
        <div className="max-w-md rounded-[2rem] bg-white p-8 shadow-[0_20px_50px_-25px_rgba(35,20,12,0.35)] ring-1 ring-black/5">
          ...
          <p className="mt-3 text-sm font-bold leading-relaxed text-[#704322]/60">Đăng nhập để chat trực tiếp với nhà hàng.</p>
    ...
    <div className="min-h-screen bg-[#fffcf8] pt-32 pb-24 lg:pt-40">
      <div className="mx-auto max-w-[1300px] px-4 sm:px-6 lg:px-10">
        <header className="mb-8 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <div>
            <p className="text-xs font-black uppercase tracking-widest text-[#ff6b00]">Tin nhắn</p>
            <h1 className="mt-2 text-4xl font-black tracking-tight text-[#23140c] sm:text-6xl">Chat với nhà hàng</h1>
          ...
          <Link href="/restaurants" className="inline-flex h-12 items-center justify-center gap-2 rounded-[1rem] bg-[#23140c] px-5 text-sm font-black text-white transition-all hover:bg-[#ff6b00] active:scale-95">
        ...
        <section className="grid h-[min(720px,calc(100dvh-11rem))] min-h-[560px] overflow-hidden rounded-[2rem] bg-white shadow-[0_20px_55px_-32px_rgba(35,20,12,0.45)] ring-1 ring-black/5 lg:grid-cols-[360px_1fr]">
          <aside className="no-scrollbar min-h-0 overflow-y-auto border-b border-[#23140c]/5 bg-[#fff7ed] p-4 lg:border-b-0 lg:border-r">
            <div className="mb-4 flex items-center justify-between">
              ...
              <span className="rounded-full bg-white px-3 py-1 text-xs font-black text-[#704322]/60 ring-1 ring-black/5">{conversations.length}</span>
            </div>
            ...
            {isLoadingConversations ? (
              ...
              <div className="rounded-[1.5rem] border border-dashed border-[#23140c]/10 bg-white px-5 py-12 text-center">
                <Storefront size={40} weight="bold" className="mx-auto text-orange-200" />
                <h3 className="mt-4 text-lg font-black text-[#23140c]">Chưa có hội thoại</h3>
                <p className="mt-2 text-sm font-bold leading-relaxed text-[#704322]/55">Vào trang chi tiết nhà hàng để bắt đầu chat.</p>
              </div>
            ) : (
                  ...
                  return (
                    ...
                    >
                      <div className={`grid size-12 shrink-0 place-items-center rounded-[1rem] ${isActive ? "bg-white/10 text-white" : "bg-orange-50 text-[#ff6b00]"}`}>
                        {conversation.userId === user?.id ? <Storefront size={24} weight="bold" /> : <UserCircle size={24} weight="bold" />}
                      </div>
                      <div className="min-w-0 flex-1">
                        <p className="truncate text-sm font-black">{conversationTitle(conversation, user?.id)}</p>
                        <p className={`mt-1 truncate text-xs font-bold ${isActive ? "text-white/55" : "text-[#704322]/50"}`}>
                          {conversation.lastMessage || conversationSubtitle(conversation, user?.id)}
                        </p>
                      </div>
            ...
            {selectedConversation ? (
              <>
                <div className="flex items-center justify-between gap-4 border-b border-[#23140c]/5 px-5 py-4">
                  <div className="min-w-0">
                    <h2 className="truncate text-xl font-black text-[#23140c]">{conversationTitle(selectedConversation, user?.id)}</h2>
                  ...
                  <Link href={`/restaurants/${selectedConversation.restaurantId}`} className="hidden h-11 items-center justify-center rounded-[1rem] bg-orange-50 px-4 text-xs font-black text-[#ff6b00] ring-1 ring-orange-100 transition-all hover:bg-orange-100 active:scale-95 sm:inline-flex">
                    Xem quán
                  </Link>
                </div>
                  ...
                  {isLoadingMessages ? (
                    ...
                    <div className="flex h-full min-h-[360px] items-center justify-center text-center">
                      <div className="max-w-sm">
                        ...
                        <h3 className="mt-4 text-xl font-black text-[#23140c]">Bắt đầu cuộc trò chuyện</h3>
                        <p className="mt-2 text-sm font-bold leading-relaxed text-[#704322]/55">Gửi lời nhắn đầu tiên để nhà hàng biết bạn cần hỗ trợ gì.</p>
                      </div>
                    </div>
                  ) : (
              ...
              <div className="flex min-h-0 flex-1 items-center justify-center p-8 text-center">
                <div className="max-w-sm">
                  ...
                  <h2 className="mt-4 text-2xl font-black text-[#23140c]">Chọn một hội thoại</h2>
                  <p className="mt-2 text-sm font-bold leading-relaxed text-[#704322]/55">Danh sách tin nhắn sẽ xuất hiện ở đây khi bạn chọn nhà hàng hoặc khách hàng.</p>
                </div>
              </div>
            )}
          </div>
