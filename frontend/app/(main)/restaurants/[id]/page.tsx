...
const RESTAURANT_FALLBACK_IMAGE =
  "https://picsum.photos/seed/hungerdash-detail/1400/700";
...
function buildRestaurantImage(restaurant: RestaurantResponse) {
  ...
  return RESTAURANT_FALLBACK_IMAGE.replace(
    "hungerdash-detail",
...
export default function RestaurantDetailPage({
  ...
  const handleFavoriteChange = (resId: number, nextFavorite: boolean) => {
    queryClient.setQueryData<boolean>(
      ["favorite", "restaurant", resId],
      nextFavorite,
    );
    queryClient.invalidateQueries({ queryKey: ["favoriteRestaurantIds"] });
  ...
  const favoriteQuery = useQuery({
    queryKey: ["favorite", "restaurant", restaurantId],
    queryFn: () => checkFavoriteRestaurant(restaurantId),
    enabled: isAuthenticated && !isAuthLoading && !Number.isNaN(restaurantId),
  });

  const isLoading = restaurantQuery.isLoading || dishesQuery.isLoading;
  ...
  const categories = useMemo(() => {
    ...
    return allCats.filter((cat) => cat.restaurantId === restaurantId);
  }, [categoriesQuery.data, restaurantId]);
  ...
  const categoryTabs = useMemo(() => {
    const categoryIdsFromDishes = Array.from(
      new Set(dishes.map((dish) => dish.categoryId)),
    );
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-[#fffcf8] px-4">
        <div className="max-w-md rounded-4xl bg-white p-8 text-center shadow-[0_20px_50px_-25px_rgba(35,20,12,0.35)] ring-1 ring-black/5">
          ...
          <h1 className="text-2xl font-black tracking-tight text-[#23140c]">
            Không tải được nhà hàng
          </h1>
              ...
              {filteredDishes.length === 0 ? (
                <div className="rounded-4xl border border-dashed border-[#23140c]/10 bg-white px-6 py-20 text-center">
                  ...
                  <p className="mt-3 text-sm font-bold text-[#704322]/55">
                    Nhà hàng này chưa cập nhật món trong danh mục đang chọn.
                  </p>
                </div>
              ) : (
                <div className="grid gap-6 md:grid-cols-2">
                        ...
                        <div className="flex min-w-0 flex-1 flex-col justify-between">
                          <div>
                            <div className="flex items-start justify-between gap-3">
                              <h4 className="line-clamp-2 font-black leading-tight text-[#23140c]">
                                {dish.name}
                              </h4>
                          ...
                          <div className="flex items-center justify-end">
                            {quantity > 0 ? (
                              <div className="flex items-center gap-3 rounded-xl bg-[#23140c] p-1 shadow-lg">
                                ...
                                <button
                                  disabled={
                                    !dish.isAvailable || !restaurant.isOpen
                                  }
                                  onClick={() =>
                                    addToCart({
                                      id: dish.id,
        ...
        <section className="mt-14 rounded-4xl bg-white p-5 shadow-[0_20px_50px_-28px_rgba(35,20,12,0.28)] ring-1 ring-black/5 sm:p-7">
          <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div>
              ...
              <h2 className="mt-1 text-3xl font-black tracking-tight text-[#23140c]">
            ...
            <div className="mb-6 flex gap-2 overflow-x-auto pb-1">
              {reviewRatingOptions.map((option) => {
                const count =
                  option === "all"
                    ? reviews.length
                    : reviews.filter((review) => review.rating === option)
                        .length;
                return (
                  <button
                    key={option}
          ...
          {reviews.length === 0 ? (
            <div className="rounded-3xl border border-dashed border-[#23140c]/10 px-6 py-16 text-center">
              ...
              <p className="mt-2 text-sm font-bold text-[#704322]/55">
                Các đánh giá sau khi khách nhận hàng sẽ xuất hiện tại đây.
              </p>
            </div>
            ...
            <div className="rounded-3xl border border-dashed border-[#23140c]/10 px-6 py-16 text-center">
              ...
              <p className="mt-2 text-sm font-bold text-[#704322]/55">
                Thử chọn mức sao khác để xem thêm đánh giá.
              </p>
            </div>
          ) : (
              ...
              {filteredReviews.slice(0, 6).map((review) => (
                ...
                >
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <span className="text-[10px] font-black uppercase tracking-widest text-[#704322]/35">
                      Đơn #{review.orderId}
                    </span>
                  </div>
