...
export default function ForgotPasswordPage() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const message = caughtError instanceof ApiError ? caughtError.message : "Không thể gửi yêu cầu lúc này.";
      setError(message);
    } finally {
    ...
    <main className="min-h-dvh bg-[#fff7ed] px-4 py-12">
      <section className="mx-auto flex min-h-[70dvh] max-w-xl flex-col justify-center">
        ...
        <div className="rounded-[2rem] bg-white p-6 shadow-[0_30px_80px_-40px_rgba(35,20,12,0.35)] ring-1 ring-black/5 sm:p-10">
          <h1 className="text-4xl font-black tracking-tight text-[#23140c]">Quên mật khẩu</h1>
          <p className="mt-3 font-medium text-[#704322]/70">
            Nhập email tài khoản, hệ thống sẽ tạo link đặt lại mật khẩu cho bạn.
          </p>

          <form className="mt-8 space-y-5" onSubmit={handleSubmit}>
            <div className="space-y-2">
              <label htmlFor="forgot-email" className="text-sm font-black uppercase tracking-widest text-[#23140c]/40">Email</label>
              <div className="relative">
                <EnvelopeSimple size={24} className="absolute left-5 top-1/2 -translate-y-1/2 text-[#704322]/40" />
                <input
                  id="forgot-email"
                  type="email"
                  value={email}
