  ...
  useEffect(() => {
    ...
    async function loadOrders() {
      ...
      try {
        ...
        const restaurantIds = Array.from(new Set(data.map((order) => order.restaurantId)));
        ...
        if (isCurrentRequest) {
          setOrders(data);
          setRestaurantsById(Object.fromEntries(restaurants.filter(Boolean).map((restaurant) => [restaurant!.id, restaurant!])));
        }
      } catch (error) {
        if (isCurrentRequest) {
    ...
    return (
      <div className="flex min-h-[80dvh] items-center justify-center px-4 text-center">
        <div className="max-w-md rounded-[2rem] bg-white p-8 shadow-[0_20px_50px_-25px_rgba(35,20,12,0.35)] ring-1 ring-black/5">
          ...
          <p className="mt-3 text-sm font-bold leading-relaxed text-[#704322]/60">Đăng nhập để xem trạng thái đơn hàng của bạn.</p>
    ...
    <div className="min-h-screen bg-[#fffcf8] pt-32 pb-24 lg:pt-40">
      <div className="mx-auto max-w-[1200px] px-4 sm:px-6 lg:px-10">
        <header className="mb-10 flex flex-col gap-5 md:flex-row md:items-end md:justify-between">
          <div>
            <Link href="/restaurants" className="mb-4 inline-flex items-center gap-2 text-sm font-bold text-[#ff6b00] hover:text-[#e45f00]">
              <ArrowLeft size={18} weight="bold" />
              Tiếp tục mua sắm
            </Link>
            <h1 className="text-5xl font-black tracking-tight text-[#23140c] sm:text-6xl">Đơn hàng</h1>
            <p className="mt-4 max-w-2xl text-sm font-bold leading-relaxed text-[#704322]/60">Theo dõi trạng thái xác nhận, chuẩn bị và giao hàng.</p>
          </div>
          <div className="rounded-[1.5rem] bg-white px-5 py-4 ring-1 ring-black/5">
            <p className="text-3xl font-black text-[#23140c]">{activeOrders.length}</p>
            <p className="text-xs font-black uppercase tracking-widest text-[#704322]/50">Đơn đang xử lý</p>
          </div>
        </header>
        ...
        {isLoading ? (
          ...
          <div className="rounded-[2rem] border border-dashed border-[#23140c]/10 bg-white px-6 py-24 text-center">
            ...
            <h2 className="text-2xl font-black text-[#23140c]">Chưa có đơn hàng</h2>
            <p className="mt-3 text-sm font-bold text-[#704322]/55">Khi bạn đặt hàng, trạng thái đơn sẽ xuất hiện tại đây.</p>
          ...
          <div className="space-y-6">
            {orders.map((order) => {
              ...
              return (
                <article key={order.id} className="rounded-4xl bg-white p-5 shadow-[0_20px_50px_-28px_rgba(35,20,12,0.28)] ring-1 ring-black/5 sm:p-7">
                    ...
                    <div className="flex flex-wrap gap-3">
                      ...
                      {canReceive && (
                        ...
                        >
                          ...
                          {receivingId === order.id ? "Đang xác nhận..." : "Đã nhận hàng"}
                        </button>
                      )}
                      {canReview && (
                        <Link
