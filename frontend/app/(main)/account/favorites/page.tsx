...
function buildImageUrl(restaurant: RestaurantResponse) {
  ...
  return FALLBACK_IMAGE.replace(
    "hungerdash-favorite",
    `favorite-restaurant-${restaurant.id}`,
  );
}
...
export default function FavoriteRestaurantsPage() {
  ...
  const handleFavoriteChange = (restaurantId: number, isFavorite: boolean) => {
    if (!isFavorite) {
      queryClient.setQueryData<RestaurantResponse[]>(
        ["favoriteRestaurants"],
        (oldData) => {
          if (!oldData) return [];
      ...
      <div className="flex min-h-[80dvh] items-center justify-center bg-[#fffcf8] px-4 text-center">
        <div className="max-w-md rounded-4xl bg-white p-8 shadow-[0_20px_50px_-25px_rgba(35,20,12,0.35)] ring-1 ring-black/5">
          ...
          <p className="mt-3 text-sm font-bold leading-relaxed text-[#704322]/60">
            Đăng nhập để xem các nhà hàng đã yêu thích.
          </p>
          <Link
            href="/login"
        ...
        <header className="mb-10 flex flex-col gap-5 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <p className="mt-4 max-w-2xl text-sm font-bold leading-relaxed text-[#704322]/60">
              Những quán bạn đã lưu để quay lại nhanh hơn trong lần đặt món tiếp
              theo.
            </p>
          </div>
        ...
        {isLoading ? (
          <div className="grid gap-6 md:grid-cols-2">
            {Array.from({ length: 4 }).map((_, index) => (
              <div
                key={index}
                className="h-72 animate-pulse rounded-4xl bg-white ring-1 ring-black/5"
              />
            ))}
          </div>
          ...
          <div className="rounded-4xl border border-dashed border-[#23140c]/10 bg-white px-6 py-24 text-center">
            ...
            <p className="mt-3 text-sm font-bold text-[#704322]/55">
              Bấm trái tim trên nhà hàng để lưu vào danh sách này.
            </p>
            <Link
              href="/restaurants"
              className="mt-7 inline-flex h-12 items-center justify-center rounded-2xl bg-[#23140c] px-6 text-sm font-black text-white transition-all hover:bg-[#ff6b00] active:scale-95"
          ...
          <div className="grid gap-6 md:grid-cols-2">
            {restaurants.map((restaurant) => (
              <article
                key={restaurant.id}
                className="overflow-hidden rounded-4xl bg-white p-4 shadow-[0_18px_45px_-28px_rgba(35,20,12,0.45)] ring-1 ring-[#23140c]/5"
              >
                <div className="relative h-56 overflow-hidden rounded-3xl bg-[#f1e7dc]">
                  <img
                  ...
                  <div className="absolute left-4 top-4 flex flex-wrap gap-2">
                    <span
                      className={`rounded-full px-3 py-1.5 text-[10px] font-black uppercase tracking-widest text-white ${restaurant.isOpen ? "bg-emerald-600" : "bg-[#704322]"}`}
                    >
                ...
                <div className="p-3 pt-5">
                  <div className="flex items-start justify-between gap-4">
                    <div className="min-w-0">
                      <h2 className="line-clamp-2 text-2xl font-black tracking-tight text-[#23140c]">
                        {restaurant.name}
                      </h2>
                  ...
                  <div className="mt-5 grid gap-2 text-xs font-black text-[#704322] sm:grid-cols-2">
                    <div className="flex items-center gap-2 rounded-2xl bg-[#fff7ed] px-3 py-2.5">
                    ...
                    <div className="flex items-center gap-2 rounded-2xl bg-orange-50 px-3 py-2.5 text-[#ff6b00] sm:col-span-2">
                  ...
                  <Link
                    ...
                    className="mt-5 inline-flex h-12 w-full items-center justify-center gap-2 rounded-2xl bg-[#23140c] text-sm font-black text-white transition-all hover:bg-[#ff6b00] active:scale-95"
                  >
                    Xem thực đơn
